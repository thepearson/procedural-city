...
function permute(x) {
    return new THREE.Vector3(((x.x * 34.0) + 1.0) * x.x % 289.0, ((x.y * 34.0) + 1.0) * x.y % 289.0, ((x.z * 34.0) + 1.0) * x.z % 289.0);
}
export function snoise(v) {
    const C = new THREE.Vector4((3.0 - Math.sqrt(3.0)) / 6.0, 0.5 * (Math.sqrt(3.0) - 1.0), -1.0 + 2.0 * ((3.0 - Math.sqrt(3.0)) / 6.0), 1.0 / 41.0);
    ...
    let m = new THREE.Vector3(Math.max(0.5 - (x0.x * x0.x + x0.y * x0.y), 0.0), Math.max(0.5 - (x12.x * x12.x + x12.y * x12.y), 0.0), Math.max(0.5 - (x12.z * x12.z + x12.w * x12.w), 0.0));
    m.x = m.x * m.x * m.x * m.x;
    m.y = m.y * m.y * m.y * m.y;
    m.z = m.z * m.z * m.z * m.z;
    let x = new THREE.Vector3(2.0 * (p.x * C.w % 1.0) - 1.0, 2.0 * (p.y * C.w % 1.0) - 1.0, 2.0 * (p.z * C.w % 1.0) - 1.0);
    ...
    let g = new THREE.Vector3(a0.x * x0.x + h.x * x0.y, a0.y * x12.x + h.y * x12.y, a0.z * x12.z + h.z * x12.w);
